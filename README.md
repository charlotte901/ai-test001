...
- 左侧 **探索案例** 回到第一组案例并启动轮播；当前没有接入正式测评题库或评分。**Features** 恢复第一组案例。
...
- 顶部按最新参考图换为圆润的 **TEST!** 字标：`TestWordmark.jsx` 直接保留 `test-wordmark-reference.png` 的字形并去除深色底，不使用近似字体。四张卡片的中英文名称仍原样保留。卡片可以点击/键盘选择，出现选中标记；未接入题库、作答或评分流程。测评页 **返回选择** 回到 CHOOSE! 页。
